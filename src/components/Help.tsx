...
/* ---- Teil 1: Anleitung ---- */
type Kapitel = { titel: string; text: JSX.Element };

const ANLEITUNG: Kapitel[] = [
  {
...
const TEILE = [
  ...
  { id: "tipps", label: "Lerntipps" },
  { id: "theorie", label: "Lerntheorie" },
];
...
export function Help() {
  ...
  return (
    <>
      <button className="icon-btn" onClick={() => setOpen(true)} title="Hilfe" aria-label="Hilfe">
        ...
        <div className="modal-backdrop" onClick={() => setOpen(false)}>
          <div className="modal help-modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-head">
              <div className="modal-title">Hilfe</div>
            ...
            <div className="seg help-seg" role="tablist">
              {TEILE.map((t) => (
                <button key={t.id} aria-pressed={teil === t.id} onClick={() => { setTeil(t.id); setKapitel(null); }}>{t.label}</button>
              ))}
            </div>
                ...
                kapitel == null ? (
                  ...
                  <>
                    <button className="btn btn-ghost btn-sm" onClick={() => setKapitel(null)}>
                      <Icon name="chevron-left" size={14} /> Alle Kapitel
                    </button>
                    <h3 className="help-chapter-title">{ANLEITUNG[kapitel].titel}</h3>
                    <div className="help-chapter-body">{ANLEITUNG[kapitel].text}</div>
                    <div className="modal-foot">
                      <button className="btn btn-ghost btn-sm" disabled={kapitel === 0} onClick={() => setKapitel((c) => (c || 0) - 1)}>Zurück</button>
                      <button className="btn btn-ghost btn-sm" disabled={kapitel === ANLEITUNG.length - 1} onClick={() => setKapitel((c) => (c || 0) + 1)}>Weiter</button>
                    </div>
                  </>
                )
              )}
                  ...
                  {LERN_TIPPS.map((t, i) => (
                    <div key={i} className={"help-tip" + (tipp === i ? " open" : "")}>
                      <button className="help-tip-head" onClick={() => setTipp(tipp === i ? null : i)}>
                        <span className="help-num">{i + 1}</span>
                        <span className="grow">{t.h}</span>
                        <Icon name={tipp === i ? "chevron-left" : "arrowRight"} size={14} />
                      </button>
                      {tipp === i && <p className="help-tip-body">{t.b}</p>}
                    </div>
                  ))}
                </div>
              )}
